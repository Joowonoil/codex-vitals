...
final class CodexAccountCaptureService: @unchecked Sendable {
    ...
    private func writeCapturedAuth(
        ...
        let payload: [String: Any] = [
            ...
            "auth_mode": "chatgpt",
            "last_refresh": ISO8601DateFormatter.codexVitals.string(from: Date()),
            "tokens": [
        ...
        let meta: [String: Any] = [
            ...
            "captured_at": ISO8601DateFormatter.codexVitals.string(from: Date()),
...
final class LocalAccountRemovalService: @unchecked Sendable {
    ...
    private func makeBackupURL() throws -> URL {
        let timestamp = DateFormatter.codexVitalsBackup.string(from: Date())
...
final class CodexAccountSwitchService: @unchecked Sendable {
    ...
    private func backupLiveAuthBeforeSwitch(to profile: CapturedCodexProfile) throws {
        ...
        try AppStorage.writeJSON(
            [
                ...
                "created_at": ISO8601DateFormatter.codexVitals.string(from: Date()),
    ...
    private func run(_ launchPath: String, _ arguments: [String], timeout: TimeInterval = 10) throws -> String {
        ...
        let outputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("CodexVitals-\(UUID().uuidString).log")
        guard fileManager.createFile(atPath: outputURL.path, contents: nil),
              let outputHandle = FileHandle(forWritingAtPath: outputURL.path) else {
            throw NSError(
                domain: "CodexVitals.CodexAccountSwitchService",
                code: 1,
...
private final class OAuthCallbackServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "CodexVitals.OAuthCallback")
...
private extension String {
    ...
    var urlFormEncoded: String {
        addingPercentEncoding(withAllowedCharacters: .codexVitalsFormAllowed) ?? self
    }
}

private extension CharacterSet {
    static let codexVitalsFormAllowed: CharacterSet = {
...
extension ISO8601DateFormatter {
    static var codexVitals: ISO8601DateFormatter {
...
extension DateFormatter {
    static var codexVitalsBackup: DateFormatter {
