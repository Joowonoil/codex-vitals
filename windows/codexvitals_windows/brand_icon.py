...
ICON_ASSET_NAME = "CodexVitals-1024.png"
RAMTER_STUDIO_LOGO_ASSET_NAME = "RamterStudioLogo.png"


def _bundled_asset_path(asset_name: str) -> Path:
    bundle_root = getattr(sys, "_MEIPASS", None)
    if bundle_root:
        return Path(bundle_root) / "build-assets" / asset_name
    return Path(__file__).resolve().parents[1] / "build-assets" / asset_name


def codex_vitals_icon_path() -> Path:
    return _bundled_asset_path(ICON_ASSET_NAME)


def ramter_studio_logo_path() -> Path:
    return _bundled_asset_path(RAMTER_STUDIO_LOGO_ASSET_NAME)


@lru_cache(maxsize=12)
def _cached_codex_vitals_icon(size: int) -> Image.Image:
    with Image.open(codex_vitals_icon_path()) as source:
        return source.convert("RGBA").resize((size, size), Image.Resampling.LANCZOS)


def build_codex_vitals_icon(size: int) -> Image.Image:
    if size <= 0:
        raise ValueError("Icon size must be positive.")
    return _cached_codex_vitals_icon(size).copy()


@lru_cache(maxsize=12)
def _cached_ramter_studio_logo(width: int, height: int, color: str) -> Image.Image:
    with Image.open(ramter_studio_logo_path()) as source:
        source_alpha = source.convert("RGBA").getchannel("A")

    scale = min(width / source_alpha.width, height / source_alpha.height)
    fitted_size = (
        max(1, round(source_alpha.width * scale)),
        max(1, round(source_alpha.height * scale)),
    )
    alpha = source_alpha.resize(fitted_size, Image.Resampling.LANCZOS)
    template = Image.new("RGBA", fitted_size, color)
    template.putalpha(alpha)

    image = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    image.alpha_composite(template, (0, (height - fitted_size[1]) // 2))
    return image


def build_ramter_studio_logo(width: int, height: int, color: str) -> Image.Image:
    if width <= 0 or height <= 0:
        raise ValueError("Logo dimensions must be positive.")
    return _cached_ramter_studio_logo(width, height, color).copy()
