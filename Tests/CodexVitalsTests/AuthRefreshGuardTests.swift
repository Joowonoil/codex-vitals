...
final class AuthRefreshGuardTests: XCTestCase {
    ...
    func testVitalsPersistsLatestAuthBeforeNormalTermination() throws {
        let root = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let mainURL = root.appendingPathComponent("Sources/CodexVitals/main.swift")
        ...
        guard let terminationRange = text.range(of: "func applicationWillTerminate"),
            ...
            return XCTFail("Could not locate the Vitals termination handoff")
        }
    ...
    func testAccountProfileStoreMethodsHoldAuthFileLock() throws {
        let root = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let storageURL = root.appendingPathComponent("Sources/CodexVitals/AppStorage.swift")
        let text = try String(contentsOf: storageURL, encoding: .utf8)

        for method in ["func upsert(", "func updateTokens(", "func updateAlias(", "func updateWorkspaceAlias(", "func updateDefaultOrder(", "func remove(profileKeys:"] {
    ...
    func testCapturedProfileDedupeDoesNotIgnoreDeleteFailures() throws {
        let root = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let dedupeURL = root.appendingPathComponent("Sources/CodexVitals/CapturedProfileDedupeService.swift")
    ...
    func testBackgroundTokenRefreshIsNotConfigured() throws {
        let root = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let mirrorURL = root.appendingPathComponent("Sources/CodexVitals/CodexAuthMirrorService.swift")
